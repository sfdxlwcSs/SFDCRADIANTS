...
import getContacts from '@salesforce/apex/Mpledge_Assesmemt.getContacts';
...
export default class ApexContactsForAccount extends LightningElement {
    ...
    contactsResults({
        ...
        if (data) {
            ...
            this.badgeLabel = 'Related Contacts(' + this.noOfContacts + ')';
        } else if (error) {
            // this.contacts = error;
            this.errorSearch = JSON.stringify(error);
        }
    }

    get showData() {

        let hasData = this.contacts.length > 0 ? true : false;
        return hasData;
    }
